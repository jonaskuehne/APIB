/**
 * @file APIB_unpacking.cpp
 * @author Jonas Kühne (jonas.kuehne@proton.me)
 * @brief Contains implementations of specializations of unpacking functions
 * 
 */
...
void madd_unpack(buf_type* buf, base_type* packed_data, Packed_Loc loc, int row_num, int col_num, int buf_cols, int num_bits) {
    if (CHECK_TEMPLATE) printf("madd 32bit unpacking\n");
        ...
        for (row = 0; row < row_num - 1; row+=2) {
            ...
            if (col_loc.bit_offset >= bits_type) {
                ...
                if (col_loc.bit_offset > bits_type) {
                    ...
                    // add to result
                    d_vec_0_0 = _mm512_or_si512(d_vec_0_0, m_sl_p_vec_0);
                    d_vec_1_0 = _mm512_or_si512(d_vec_1_0, m_sl_p_vec_1);
                    d_vec_2_0 = _mm512_or_si512(d_vec_2_0, m_sl_p_vec_2);
                    d_vec_3_0 = _mm512_or_si512(d_vec_3_0, m_sl_p_vec_3);
                }
            ...
            auto d_vec_0_1 = _mm512_slli_epi32(pre_d_vec_0_1, 16);
            auto d_vec_1_1 = _mm512_slli_epi32(pre_d_vec_1_1, 16);
            auto d_vec_2_1 = _mm512_slli_epi32(pre_d_vec_2_1, 16);
            auto d_vec_3_1 = _mm512_slli_epi32(pre_d_vec_3_1, 16);
...
template void madd_unpack<uint16_t, uint32_t>(uint16_t* buf, uint32_t* packed_data, Packed_Loc loc, int row_num, int col_num, int buf_cols, int num_bits);
...
void amx_unpack(buf_type* buf, base_type* packed_data, Packed_Loc loc, int row_num, int col_num, int buf_cols, int num_bits) {
    ...
    for (col=0; col < col_num - 31; col+=32) {
        ...
        for (row = 0; row < row_num - 3; row+=4) {
            ...
            auto d_vec_0_1 = _mm512_slli_epi32(pre_d_vec_0_1, 8);
            auto d_vec_1_1 = _mm512_slli_epi32(pre_d_vec_1_1, 8);
            ...
            auto d_vec_0_2 = _mm512_slli_epi32(pre_d_vec_0_2, 16);
            auto d_vec_1_2 = _mm512_slli_epi32(pre_d_vec_1_2, 16);
            ...
            auto d_vec_0_3 = _mm512_slli_epi32(pre_d_vec_0_3, 24);
            auto d_vec_1_3 = _mm512_slli_epi32(pre_d_vec_1_3, 24);
        ...
        if (row < row_num) {
            ...
            for (;row < row_num; row++) {
                ...
                auto d_vec_0 = _mm512_slli_epi32(pre_d_vec_0, amnt);
                auto d_vec_1 = _mm512_slli_epi32(pre_d_vec_1, amnt);
...
// declare actual used specialization
template void amx_unpack<uint8_t, uint32_t>(uint8_t* buf, uint32_t* packed_data, Packed_Loc loc, int row_num, int col_num, int buf_cols, int num_bits);
...
void unpack(buf_type* buf, base_type* packed_data, Packed_Loc loc, int row_num, int col_num, int buf_cols, int num_bits) {
    if (CHECK_TEMPLATE) printf("generic unpacking\n");
    const int bits_type = 8 * sizeof(base_type);
    buf_type mask; 
    if (num_bits >= 64) {
        mask = (buf_type)(-1);
    } else {
        mask = (buf_type)((1L << num_bits) - 1); 
    }
...
// specialization to unpack to 32 bits from basetype of 32 bits
template<>
void unpack<uint32_t, uint32_t>(uint32_t* buf, uint32_t* packed_data, Packed_Loc loc, int row_num, int col_num, int buf_cols, int num_bits) {
    if (CHECK_TEMPLATE) printf("32bit unpacking\n");
    uint32_t mask = (uint32_t)((1L << num_bits) - 1); 
    auto mask_vec = _mm512_set1_epi32(mask);
    ...
    for (col = 0; col < col_num - 127; col+=128) {
        ...
        for (int row = 0; row < row_num; row++) {
            ...
            _mm512_storeu_si512((void*)(store_loc), d_vec_0);
            _mm512_storeu_si512((void*)(store_loc + 16), d_vec_1);
            _mm512_storeu_si512((void*)(store_loc + 32), d_vec_2);
            _mm512_storeu_si512((void*)(store_loc + 48), d_vec_3);
            _mm512_storeu_si512((void*)(store_loc + 64), d_vec_4);
            _mm512_storeu_si512((void*)(store_loc + 80), d_vec_5);
            _mm512_storeu_si512((void*)(store_loc + 96), d_vec_6);
            _mm512_storeu_si512((void*)(store_loc + 112), d_vec_7);
        }
    }
...
// specialization to unpack to 16 bits from basetype of 32 bits
template<>
void unpack<uint16_t, uint32_t>(uint16_t* buf, uint32_t* packed_data, Packed_Loc loc, int row_num, int col_num, int buf_cols, int num_bits) {
    if (CHECK_TEMPLATE) printf("16bit unpacking\n");
    uint32_t mask = (uint32_t)((1L << num_bits) - 1); 
    auto mask_vec = _mm512_set1_epi32(mask);
    const int bits_type = 32;
    ...
    for (col = 0; col < col_num - 127; col+=128) {
        ...
        for (int row = 0; row < row_num; row++) {
            ...
            _mm512_storeu_si512((void*)(store_loc + 32), aligned_1);
            _mm512_storeu_si512((void*)(store_loc + 64), aligned_2);
            _mm512_storeu_si512((void*)(store_loc + 96), aligned_3);
...
// specialization to unpack to 8 bits from basetype of 32 bits
template<>
void unpack<uint8_t, uint32_t>(uint8_t* buf, uint32_t* packed_data, Packed_Loc loc, int row_num, int col_num, int buf_cols, int num_bits) {
    ...
    for (col = 0; col < col_num - 127; col+=128) {
        ...
        for (int row = 0; row < row_num; row++) {
            ...
            auto packed_0 = _mm512_packus_epi16(packed_16_0_0, packed_16_1_0);
            auto packed_1 = _mm512_packus_epi16(packed_16_0_1, packed_16_1_1);
...
// specialization to unpack to 64 bits from basetype of 32 bits
template<>
void unpack<uint64_t, uint32_t>(uint64_t* buf, uint32_t* packed_data, Packed_Loc loc, int row_num, int col_num, int buf_cols, int num_bits) {
    if (CHECK_TEMPLATE) printf("64bit unpacking\n");
    uint32_t mask = (uint32_t)((1L << num_bits) - 1); 
    auto mask_vec = _mm512_set1_epi32(mask);
    ...
    for (col = 0; col < col_num - 63; col+=64) {
        ...
        for (int row = 0; row < row_num; row++) {
            ...
            _mm512_storeu_si512((void*)(store_loc), d_vec_0_0_64);
            _mm512_storeu_si512((void*)(store_loc + 8), d_vec_1_0_64);
            _mm512_storeu_si512((void*)(store_loc + 16), d_vec_0_1_64);
            _mm512_storeu_si512((void*)(store_loc + 24), d_vec_1_1_64);
            _mm512_storeu_si512((void*)(store_loc + 32), d_vec_0_2_64);
            _mm512_storeu_si512((void*)(store_loc + 40), d_vec_1_2_64);
            _mm512_storeu_si512((void*)(store_loc + 48), d_vec_0_3_64);
            _mm512_storeu_si512((void*)(store_loc + 56), d_vec_1_3_64);
        }
    }
...
// specialization to unpack to 64 bits from basetype of 64 bits
template<>
void unpack<uint64_t, uint64_t>(uint64_t* buf, uint64_t* packed_data, Packed_Loc loc, int row_num, int col_num, int buf_cols, int num_bits) {
    if (CHECK_TEMPLATE) printf("64bit to 64bit unpacking\n");
    uint64_t mask;
    if (num_bits >= 64) {
        mask = (uint64_t)(-1);
    } else if (num_bits >= 63) {
    ...
    for (col = 0; col < col_num - 63; col+=64) {
        ...
        for (int row = 0; row < row_num; row++) {
            ...
            uint64_t* store_loc = buf + row*buf_cols + col;
            _mm512_storeu_si512((void*)(store_loc), d_vec_0);
            _mm512_storeu_si512((void*)(store_loc + 8), d_vec_1);
            _mm512_storeu_si512((void*)(store_loc + 16), d_vec_2);
            _mm512_storeu_si512((void*)(store_loc + 24), d_vec_3);
            _mm512_storeu_si512((void*)(store_loc + 32), d_vec_4);
            _mm512_storeu_si512((void*)(store_loc + 40), d_vec_5);
            _mm512_storeu_si512((void*)(store_loc + 48), d_vec_6);
            _mm512_storeu_si512((void*)(store_loc + 56), d_vec_7);
        }
    }
